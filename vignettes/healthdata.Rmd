...
The WHO dataset contains agreements from the World Health Organization, and the GHHR dataset contains data from the Global Health and Human Rights database.
The HUGGO dataset is a consolidated dataset of the health agreements identified in the WHO and GHHR datasets, but improves on the precision of dates, resolves any conflicts between these datasets, and fills in any missing information that could be found.
...
Extending from data in the agreements datacube, the `memberships` datacube contains 1 dataset (HUGGO_MEM) with hand-coded data of 39000 observations on state memberships to international health instruments.
...
The **memberships** datacube contains hand-coded data on states' memberships to instruments governing the global health sphere. The `HUGGO_MEM` dataset includes specific adoption/signature, ratification, entry into force, and termination dates for each state during its membership to an agreement.
...
The graph below shows that there is a high degree of overlap in the formal international health agreements joined by some of PAHO's largest state members (Argentina, Brazil, Canada, Mexico, Peru, Venezuela, and the United States) in the 2000s.
...
The **organizations** datacube lists different actors playing a role in the global health governance, such as NGOs, IGOs or associations, with their date of establishment and headquarters location.
...
For access to more data and information in our other "many" packages, please see [manydata](https://github.com/globalgov/manydata).
