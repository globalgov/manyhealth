...
Contributions to `{manyhealth}`, whether in the form of issue identification, bug fixes, new code or documentation are encouraged and welcome, both from research assistants and (early) users of the package:

* [Submit an issue](#issues)
* [Fix a bug or implement new features](#adding-new-code)
* [Document existing code](#documentation)

This outlines how to propose a change to a package from the Global Governance Observatory's ecosystem. Please note that the `manyhealth` project is released with a [Contributor Code of Conduct](CODE_OF_CONDUCT.md).
...
lintr::lint_package(path = "manyhealth/")

# goodpractices checks. Exclude length 80
goodpractice::gp(path = "manyhealth/",
...
Make sure the package passes R CMD check by running `devtools::check()` before committing changes to a pull request. 

If you want to make a bigger change, it's a good idea to first file an issue and make sure someone from the team agrees that it’s needed before opening a pull request. If you’ve found a bug, please file an issue that illustrates the bug with a minimal [reprex](https://www.tidyverse.org/help/#reprex).

Please follow the manyhealth pull request guideline (https://github.com/globalgov/manyhealth/blob/main/.github/pull_request_template.md)  
...
By convention, tests are located in [testthat/tests/](manyhealth/tests/testthat).
...
- The [naming convention](https://www.tidyverse.org/articles/2019/04/testthat-2-1-0/) for test files is: ``test-FILENAME_IN_R_DIRECTORY-FUNCTION_NAME.R``, i.e. test files are named after the file containing the original function in the [R](manyhealth/R) directory, pre-fixed with "test", and optionally post-fixed with the name of the function that is being tested.
